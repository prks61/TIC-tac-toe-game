...
def ButtonClick(id):
    ...
    if( bu1['text']=='X' and bu2['text']=='X' and bu3['text']=='X' or
        ...
        bu3['text']=='X' and bu5['text']=='X' and bu7['text']=='X'):
            disableButton()
            c=1
            tkinter.messagebox.showinfo('Game over',"Winner is player 1")
        ...
        bu3['text']=='O' and bu5['text']=='O' and bu7['text']=='O'):
            disableButton()
            c=1
            tkinter.messagebox.showinfo('Game over',"Winner is player 2")
    elif b==9:
            disableButton()
            c=1
            tkinter.messagebox.showinfo('Game over',"Match is Draw.")
...
game=Tk()
# forms the window or frame
game.title("Tic Tac Toe")
# adding the buttons to the game
bu1=ttk.Button(game,text=' ')
#  pixels to pad widget, horizontally and vertically, inside widget's borders.
# lambda is a keyword used to define functions in one line.
bu1.grid(row=0,column=0,sticky='snew',ipadx=40,ipady=40)
# config function calls the buttonclick function once bu1 is presses
bu1.config(command=lambda: ButtonClick(1))
# creating button 1 for 3*3 matrix
bu2=ttk.Button(game,text=' ')
bu2.grid(row=0,column=1,sticky='snew',ipadx=40,ipady=40)
bu2.config(command=lambda: ButtonClick(2))
# creating button 2 for 3*3 matrix
bu3=ttk.Button(game,text=' ')
bu3.grid(row=0,column=2,sticky='snew',ipadx=40,ipady=40)
bu3.config(command=lambda: ButtonClick(3))
# creating button 3 for 3*3 matrix
bu4=ttk.Button(game,text=' ')
bu4.grid(row=1,column=0,sticky='snew',ipadx=40,ipady=40)
bu4.config(command=lambda: ButtonClick(4))

bu5=ttk.Button(game,text=' ')
bu5.grid(row=1,column=1,sticky='snew',ipadx=40,ipady=40)
bu5.config(command=lambda: ButtonClick(5))

bu6=ttk.Button(game,text=' ')
bu6.grid(row=1,column=2,sticky='snew',ipadx=40,ipady=40)
bu6.config(command=lambda: ButtonClick(6))

bu7=ttk.Button(game,text=' ')
bu7.grid(row=2,column=0,sticky='snew',ipadx=40,ipady=40)
bu7.config(command=lambda: ButtonClick(7))

bu8=ttk.Button(game,text=' ')
bu8.grid(row=2,column=1,sticky='snew',ipadx=40,ipady=40)
bu8.config(command=lambda: ButtonClick(8))

bu9=ttk.Button(game,text=' ')
bu9.grid(row=2,column=2,sticky='snew',ipadx=40,ipady=40)
bu9.config(command=lambda: ButtonClick(9))

playerturn=ttk.Label(game,text="   Player 1 turn!  ")
playerturn.grid(row=3,column=0,sticky='snew',ipadx=40,ipady=40)


playerdetails=ttk.Label(game,text="    Player 1 is X\n\n    Player 2 is O")
playerdetails.grid(row=3,column=2,sticky='snew',ipadx=40,ipady=40)

res=ttk.Button(game,text='Restart')
